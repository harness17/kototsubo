...
{
    /// <summary>
    /// 音楽 CD の JAN 一括登録フロー（入力 → プレビュー → 確定）。
    /// DVD/Game とは別系統の専門処理として独立実装しており、
    /// ISBN 系統（<see cref="ImportController"/>）とは共有しない。
    /// スナップショット保存・所有権トークン検証・パストラバーサル対策は
    /// 本コントローラ内で完結して実装する。
    /// </summary>
    [Authorize]
    public class CdImportController : Controller
    {
        internal const int MaxJanCount = 1000;
        private const long MaxFileSize = 1024 * 1024;

        private readonly ICdLookupService _lookup;
        private readonly ItemRepository _repository;
        private readonly ILogger<CdImportController> _logger;
        private readonly string _tempDirectory;

        public CdImportController(
            ICdLookupService lookup,
            ItemRepository repository,
            IWebHostEnvironment environment,
            ILogger<CdImportController> logger)
        {
            _lookup = lookup;
            _repository = repository;
            _logger = logger;
            _tempDirectory = Path.Combine(environment.ContentRootPath, "App_Data", "CdImports");
        }

        // 音楽 CD の JAN 入力フォームを表示する
        [HttpGet]
        public IActionResult Index()
        {
            return View(new CdImportViewModel());
        }
        ...
        [RequestFormLimits(
            ValueLengthLimit = CdImportViewModel.MaxInputLength + 1,
            MultipartBodyLengthLimit = MaxFileSize + 65536)]
        public async Task<IActionResult> Index(CdImportViewModel model)
        {
            ...
            if (model.File != null)
            {
                ...
                {
                    _logger.LogError(ex, "CD import file read failed.");
            ...
            {
                _logger.LogError(ex, "CD import preview snapshot failed.");
        ...
        /// <summary>
        /// JAN 群から音楽 CD の書誌を引き、重複判定・プレビュー行・
        /// 一時スナップショットを構築する。登録候補が 1 件以上あるときのみ
        /// スナップショットを保存し、セッションに所有権トークンを記録する。
        /// </summary>
        private async Task<CdImportPreviewViewModel> BuildPreviewAsync(
            IReadOnlyList<string> inputs)
        {
            ...
            var preview = new CdImportPreviewViewModel();
            var snapshot = new CdImportSnapshot();

            for (var i = 0; i < inputs.Count; i++)
            {
                var result = lookups[i];
                var row = new CdImportRow { Input = inputs[i], Result = result };

                if (result?.Jan == null)
                {
                    row.Error = "無効なJAN、または音楽CD情報が見つかりません。";
                }
                ...
                else
                {
                    ...
                    snapshot.Rows.Add(new CdImportSnapshotRow { Result = result });
                }
        ...
        // プレビューで選択された音楽 CD を一括登録する
        [HttpPost]
        [ValidateAntiForgeryToken]
        [RequestFormLimits(ValueCountLimit = MaxJanCount + 2)]
        public async Task<IActionResult> Confirm(CdImportPreviewViewModel model)
        {
            var result = new CdImportResultViewModel();
            string tempPath;
            try
            {
            ...
            try
            {
                ...
                var snapshot = await JsonSerializer.DeserializeAsync<CdImportSnapshot>(input)
                    ?? new CdImportSnapshot();
                ...
                foreach (var row in allowedRows)
                {
                    ...
                    {
                        _logger.LogError(ex, "CD item import failed for JAN {Jan}.", r.Jan);
            ...
            {
                _logger.LogError(ex, "CD import snapshot was invalid.");
                result.Errors.Add("確認データを読み込めませんでした。もう一度検索してください。");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "CD import confirmation failed.");
                result.Errors.Add("CD一括登録中にエラーが発生しました。");
            }
            finally
            {
                DeleteTempFile(tempPath);
        ...
        /// <summary>音楽 CD の検索結果から蔵品エンティティを生成する（MediaType は Music 固定）。</summary>
        internal static ItemEntity ToEntity(CdLookupResult r)
        {
        ...
        private static string? GetValidationError(CdLookupResult r)
        {
            if (string.IsNullOrWhiteSpace(r.Title))
                return "タイトルを取得できません。";
            if (r.Title.Length > 500)
                return "タイトルが登録可能な長さを超えています。";
            if (r.Creator?.Length > 500)
                return "アーティスト名が登録可能な長さを超えています。";
        ...
        private static string GetSessionKey(string token) => $"CdImport:{token}";
    }
}
